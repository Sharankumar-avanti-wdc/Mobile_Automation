...
public class Test18
{
	...
	public void CopyPaste() throws InterruptedException, IOException {
		...
		try
		{
			...
			touchAction7.longPress(LongPressOptions.longPressOptions().withDuration(Duration.ofMillis(3)).withElement (ElementOption.element (CheckTest3))).release().perform();
			TimeUnit.SECONDS.sleep(10);
			//select copy option
			logger.info("Select copy option");
			System.out.println("Select copy option");
