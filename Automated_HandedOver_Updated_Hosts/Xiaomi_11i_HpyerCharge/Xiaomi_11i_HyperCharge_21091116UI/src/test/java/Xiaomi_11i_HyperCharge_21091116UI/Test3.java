...
import Xiaomi_11i_HyperCharge_21091116UI.Test3;
	...
	public void Format() throws InterruptedException, SecurityException, IOException {
		...
		try {
			...
			{ 
				...
				logger.info("Pass: Test3 Pass for Mount,Unmount and Format uSD card");
				System.out.println("Pass: Test3 Pass for Mount,Unmount and Format uSD card");
				((PressesKey) driver).pressKey(new KeyEvent().withKey(AndroidKey.BACK));
				TimeUnit.SECONDS.sleep(2);
				((PressesKey) driver).pressKey(new KeyEvent().withKey(AndroidKey.BACK));
