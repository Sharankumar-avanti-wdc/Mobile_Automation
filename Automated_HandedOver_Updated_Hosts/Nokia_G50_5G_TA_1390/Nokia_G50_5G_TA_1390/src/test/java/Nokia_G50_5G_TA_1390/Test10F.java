...
import Nokia_G50_5G_TA_1390.Test0;
...
public class Test10F
{
	...
	public void frontVideoRec() throws InterruptedException, SecurityException, IOException {
		...
		try {
			...
			//IF location permission pop-up exists select Dont Allow option
			try 
			...
			MobileElement FrontCamera = driver.findElement(By.xpath("//*[@text='Video from front camera']"));
			FrontCamera.click();
			TimeUnit.SECONDS.sleep(5);

			//Select first resolution
			logger.info("Select first highest front camera video resolution: FHD 1080P/30fps");
			System.out.println("Select first highest front camera video resolution: FHD 1080P/30fps");
			MobileElement FirstRes = driver.findElement(By.xpath("//*[@text='FHD 1080P/30fps']"));
			FirstRes.click();
			TimeUnit.SECONDS.sleep(2);
			((PressesKey) driver).pressKey(new KeyEvent().withKey(AndroidKey.BACK));
			TimeUnit.SECONDS.sleep(2);
