...
public class Test18
{
	...
	public void CopyPaste() throws InterruptedException, IOException {
		...
		try {
			...
			touchAction5.longPress(LongPressOptions.longPressOptions().withDuration(Duration.ofMillis(3)).withElement (ElementOption.element (Test2))).release().perform();
			TimeUnit.SECONDS.sleep(10);
			// click on copy icon
			logger.info("Select Copy option");
			System.out.println("Select Copy option");
