...
public class Test0 {
	...
	public void start() throws 	IOException, InterruptedException  {
		...
		try {
			...
			//command to fetch recent apps/apps present in background
	...
	public void Images() throws  IOException, InterruptedException {
		...
		try {
			...
			//Switch to rear camera mode if current existing camera is Front camera.
			try {
			...
			//if camera is already in rear camera no switch required
			catch(Exception exp)
			{ 
	...
	public void Video() throws  IOException, InterruptedException {
		...
		try {
			...
			//if camera is already in rear camera mode no switch required.
			catch(Exception exp)
			{ 
			...
			logger.info("Select first highest rear camera video resolution : BACK_CAMCORDER_RESOLUTION_UHD_30");
			System.out.println("Select first highest rear camera video resolution : BACK_CAMCORDER_RESOLUTION_FHD_30");
			...
			logger.info("Select second highest rear camera video resolution : BACK_CAMCORDER_RESOLUTION_UHD_30");
			System.out.println("Select second highest rear camera video resolution : BACK_CAMCORDER_RESOLUTION_FHD_30");
	...
	public void Format() throws  IOException, InterruptedException  {	
		...
		java.util.logging.Logger logger =  java.util.logging.Logger.getLogger(this.getClass().getName());
		//Launching home screen mode
		logger.info("Launch home screeen mode");
		System.out.println("Launching home screen mode");
		((StartsActivity) driver).startActivity(new Activity("com.sec.android.app.launcher", "com.sec.android.app.launcher.activities.LauncherActivity"));
		TimeUnit.SECONDS.sleep(5);
		// open the recently opened apps screen
		((PressesKey) driver).pressKeyCode(AndroidKeyCode.KEYCODE_APP_SWITCH);
		TimeUnit.SECONDS.sleep(5);
		// clearing recent apps from background
		logger.info("Clearing recent apps from background");
		System.out.println("Clearing recent apps from background");
		try
		{
			((FindsByAndroidUIAutomator<MobileElement>) driver).findElementByAndroidUIAutomator("new UiSelector().text(\"Close all\")").click();
			TimeUnit.SECONDS.sleep(5);
		...
		try {
			...
			try {
				...
				TimeUnit.SECONDS.sleep(5);
				//Select Page 2 to select uSD card
				...
				logger.info("Accept format pop-up option");
				System.out.println("Accept format pop-up option");	
