...
import Xiaomi_Redmi_10Prime_2022_21061119B1.Test3;
	...
	public void Format() throws InterruptedException, SecurityException, IOException {
		...
		try {
			...
			{ 
				...
				logger.info("Pass: Test3 Pass for unmount,mount and Format uSD card is successfull");
				System.out.println("Pass: Test3 Pass for unmount,mount and Format uSD card is successfull");
				((PressesKey) driver).pressKey(new KeyEvent().withKey(AndroidKey.BACK));
				TimeUnit.SECONDS.sleep(2);
				((PressesKey) driver).pressKey(new KeyEvent().withKey(AndroidKey.BACK));
			}
