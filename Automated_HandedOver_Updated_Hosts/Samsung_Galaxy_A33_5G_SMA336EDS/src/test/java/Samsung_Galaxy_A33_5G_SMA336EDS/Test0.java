...
public class Test0 {
	...
	public void Images() throws  IOException, InterruptedException {
		...
		try {
			...
			logger.info("if Location permisson Pop Exists during camera application launch select cancel option ");
			System.out.println("if Location permisson Pop Exists during camera application launch select cancel option ");
			try 
			{ 
				((FindsByAndroidUIAutomator<MobileElement>) driver).findElementByAndroidUIAutomator("new UiSelector().text(\"Cancel\")").click();
	...
	public void Video() throws  IOException, InterruptedException {
		...
		try {
			...
			MobileElement FirstRes = driver.findElement(By.xpath("//*[@bounds='[218,120][338,240]']"));
			FirstRes.click();
			TimeUnit.SECONDS.sleep(2);
			...
			logger.info("Select Second highest rear camera video resolution :BACK_CAMCORDER_RESOLUTION_HD_30");
			System.out.println("Select Second highest rear camera video resolution :BACK_CAMCORDER_RESOLUTION_HD_30");
