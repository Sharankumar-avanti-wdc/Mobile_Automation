...
public class Test3_Int {
			...
			{ 
				try {
					...
					try
					{
						...
						// click on SD card
						logger.info("Select android option");
						System.out.println("Select android option");
						((FindsByAndroidUIAutomator<MobileElement>) driver).findElementByAndroidUIAutomator("new UiSelector().text(\"android\")").click();
						// click on Unmount and wait for 2 secs for it to get mounted
						TimeUnit.SECONDS.sleep(5);
					}
	...
	public void FormatAsInternal() throws  IOException, InterruptedException {
		...
		logger.info("Formating as internal mount");
		System.out.println("Formating as internal mount");
		// Launch settings app
		...
		TimeUnit.SECONDS.sleep(40);
		try {
			logger.info("Accept move content if exists");
			System.out.println("Accept move content if exists");
		...
		try {
			...
			TimeUnit.SECONDS.sleep(5);
			MobileElement File =	driver.findElement(By.xpath("//*[@resource-id='com.google.android.apps.nbu.files:id/title']"));
			File.click();
			TimeUnit.SECONDS.sleep(5);
			((PressesKey) driver).pressKey(new KeyEvent().withKey(AndroidKey.HOME));
