package Realme_Pad_Mini_RMP2106;
import org.testng.annotations.Test;
import Realme_Pad_Mini_RMP2106.Test9_Int;
...
public class Test9_Int
{
	...
	public void SetUp() throws MalformedURLException {
		...
		dc.setCapability(MobileCapabilityType.DEVICE_NAME, "realme Pad mini");
		dc.setCapability(MobileCapabilityType.UDID, "12E230531000112");
	...
	public void RearPictureCapture() throws InterruptedException, SecurityException, IOException {
		...
		try {
			...
			//If start capture pop exists select Start capture option
			try 
			{ 
				logger.info("If allow location pop exists select Dont allow option");
				System.out.println("If allow location pop exists select Dont allow option");
				((FindsByAndroidUIAutomator<MobileElement>) driver).findElementByAndroidUIAutomator("new UiSelector().text(\"DON’T ALLOW\")").click();

			} 
			catch(Exception exp)
			{ 
				logger.info("Location access pop-up  element not found : "+exp); 
				System.out.println("Location access pop-up  element not found : "+exp); 

			}
			//Select Start capture option if exists
			try 
			{ 
				logger.info("If start capture pop exists select Start capture option");
				System.out.println("If start capture pop exists select Start capture option");
				((FindsByAndroidUIAutomator<MobileElement>) driver).findElementByAndroidUIAutomator("new UiSelector().text(\"START CAPTURE\")").click();

			} 
			catch(Exception exp)
			{ 
				logger.info("Start capture element not found : "+exp); 
				System.out.println("Start capture  element not found : "+exp); 
